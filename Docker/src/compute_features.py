...
# function to read .nii files and compute biomarker values then return csv file
...
# function to read .nii file and compute biomarker values then return the dictionary
def read_nii_mask_return_tmtv_dmax(path_img_nii):
    """
    :param input_path: Path to the .nii file
    :return: read .nii, compute TMTV, Dmax
    """

    name_x_y_z_TMTV_dmax = {}
    ...
    def compute_dmax(gt_):
        ...
        for k in range(len(props)):
            ...
            if len(props) > 1:
                ...
                convex_image = props[k].convex_image
                distance_map = distance_transform_edt(convex_image)
                max_distance = distance_map.max()    
                dist_all.append(max_distance)
            else:
                dist_all.append(0)
        ...
        return np.max(dist_all), len(dist_all)

    tmtv = compute_TMTV(gt.copy())
    dmax, count_segs = compute_dmax(gt.copy())
    name_x_y_z_TMTV_dmax['ID'] = ''
    # name_x_y_z_TMTV_dmax['ID'] = str(case_name)
    name_x_y_z_TMTV_dmax['X'] = res_pet[0]
    name_x_y_z_TMTV_dmax['Y'] = res_pet[1]
    name_x_y_z_TMTV_dmax['Z'] = res_pet[2]
    name_x_y_z_TMTV_dmax['TMTV'] = tmtv * res_pet[0] * res_pet[1] * res_pet[2]
    name_x_y_z_TMTV_dmax['Dmax'] = dmax
    name_x_y_z_TMTV_dmax['Seg_regions'] = count_segs
    # except:
    #         print(f"Error reading {path_img_nii}")
    
    print("Done !!")
    return name_x_y_z_TMTV_dmax
