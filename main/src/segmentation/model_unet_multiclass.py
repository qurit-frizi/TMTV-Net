...
class ModelUNetMulticlass(nn.Module):
    ...
    def forward(self, batch: Batch) -> Dict:
        ...
        return {
            'seg': OutputSegmentation(o, seg, criterion_fn=self.loss_fn, output_postprocessing=self.output_postprocessing),
            #'2d_ct': OutputEmbedding(ct[:, :, z_half]),
            #'2d_suv': OutputEmbedding(suv[:, :, z_half]),
            #'2d_seg': OutputEmbedding(seg[:, :, z_half]),
            #'2d_found': OutputEmbedding(self.output_postprocessing((o[:, :, z_half])).type(torch.float32)),
        }
...
class LossDiceCrossEntropyFocal:
    def __init__(self, ce_factor=0.5, gamma=2.0) -> None:
        ...
        self.dice = LossDiceMulticlass(normalization_fn=None, discard_background_loss=False, smooth=1.0)
